...
const InitializeSchema = {
  wasmURL: z.string().optional().describe(
    "URL to fetch the esbuild WASM binary from",
  ),
  wasmModule: z.string().optional().describe(
    "File path to a local .wasm file (will be compiled via WebAssembly.compile)",
  ),
  worker: z.boolean().optional().describe(
    "Run esbuild in a Web Worker (default: false)",
  ),
};
