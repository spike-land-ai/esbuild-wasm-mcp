...
const LegalCommentsEnum = z.enum([
  "none",
  "inline",
  "eof",
  "linked",
...
export const CommonSchema = {
  ...
  pure: z.array(z.string()).optional()
    .describe(
      "Function calls to mark as pure for tree shaking (e.g. [\"console.log\"])",
    ),
  keepNames: z.boolean().optional().describe(
    "Preserve .name on functions and classes",
  ),
  drop: z.array(DropEnum).optional().describe(
    "Remove console/debugger statements",
  ),
  dropLabels: z.array(z.string()).optional().describe(
    "Remove labeled statements with these labels",
  ),
  charset: CharsetEnum.optional().describe(
    "Output character set (default: ascii)",
  ),
  lineLimit: z.number().optional().describe(
    "Soft line width limit for wrapping",
  ),
  ...
  jsxFactory: z.string().optional().describe(
    "JSX factory function (e.g. React.createElement)",
  ),
  jsxFragment: z.string().optional().describe(
    "JSX fragment (e.g. React.Fragment)",
  ),
  jsxImportSource: z.string().optional().describe(
    "JSX import source for automatic runtime",
  ),
  jsxDev: z.boolean().optional().describe("Use development JSX runtime"),
  jsxSideEffects: z.boolean().optional().describe(
    "Do not treat JSX as side-effect free",
  ),
  ...
  sourcesContent: z.boolean().optional().describe(
  ...
  globalName: z.string().optional().describe(
  ...
  mangleQuoted: z.boolean().optional().describe(
    "Also mangle quoted property names",
  ),
  mangleCache: z.record(z.string(), z.union([z.string(), z.literal(false)]))
    .optional()
    .describe("Mangle cache for stable renaming across builds"),
  logLevel: LogLevelEnum.optional().describe("Log level"),
  logLimit: z.number().optional().describe(
    "Max number of log messages (0 = unlimited)",
  ),
...
export const BuildOnlySchema = {
  entryPoints: z.union([
    ...
    "Entry points: array of file paths or {outName: filePath} record",
  ),
  bundle: z.boolean().optional().describe(
    "Bundle imports into output (default: true)",
  ),
  splitting: z.boolean().optional().describe(
  ...
  entryNames: z.string().optional().describe(
    "Template for entry point output names (e.g. \"[dir]/[name]-[hash]\")",
  ),
  ...
  external: z.array(z.string()).optional().describe(
    "Package names or globs to exclude from bundle",
  ),
  packages: PackagesEnum.optional().describe(
    "How to handle packages: 'bundle' or 'external'",
  ),
  alias: z.record(z.string(), z.string()).optional()
    .describe("Package import aliases (e.g. {\"oldpkg\": \"newpkg\"})"),
  resolveExtensions: z.array(z.string()).optional()
    .describe(
      "Extensions to try when resolving (e.g. [\".tsx\", \".ts\", \".jsx\", \".js\"])",
    ),
  mainFields: z.array(z.string()).optional()
    .describe("Package.json fields to try (e.g. [\"module\", \"main\"])"),
  conditions: z.array(z.string()).optional()
    .describe(
      "Package.json export conditions (e.g. [\"development\", \"module\"])",
    ),
  preserveSymlinks: z.boolean().optional().describe("Do not resolve symlinks"),
  nodePaths: z.array(z.string()).optional().describe(
  ...
  stdin: z.object({
    ...
    resolveDir: z.string().optional().describe(
      "Directory for resolving imports",
    ),
  ...
  absWorkingDir: z.string().optional().describe(
...
export const TransformOnlySchema = {
  ...
  sourcefile: z.string().optional().describe(
    "File name for error messages and source maps",
  ),
 ...
 */
export function prepareBuildOptions<T extends Record<string, unknown>>(
  args: T,
): WithRegExpMangle<T> {
  ...
  return {
    ...
    ...(mangleProps !== undefined
      ? { mangleProps: new RegExp(mangleProps as string) }
      : {}),
    ...(reserveProps !== undefined
      ? { reserveProps: new RegExp(reserveProps as string) }
      : {}),
